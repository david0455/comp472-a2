...
class AStar():
    ...
    def moveUp(self, current_puzzle_state):
        ...
        return np.int16(self.new_state)
    ...
    def astar(self, initial_state):
        ...
        print("entered astar")
        self.open_state.put([self.h0(initial_state), 0, self.h0(initial_state), initial_state, [0, self.h0(initial_state), np.int16(initial_state)]]) # initial-state: (f(n), g(n), h(n), curr _puzzle = Starting Puzzle, Path = [tile,cost, current_puzzle])  
        ...
        while not self.open_state.empty():
            ...
            if(len(self.children.queue) > 0 ):
                for self.child in self.children.queue[:]:
                    ...
                    print(self.child_puzzle_state.tolist())
                    if (self.open_state.queue.count(self.child_puzzle_state) > 0): # check if child is in open_state
                    ...
                    elif (self.closed_state.count(self.child_puzzle_state) > 0): # check if child is in closed list
                        ...
                        if self.compare_Cost(child_f, self.child_puzzle_state, self.open_state):  # if child_f is lower than the cost inside closed list, put it back to open_state
                            self.open_state = self.replace_Cost(child_f, self.child_puzzle_state, self.closed_state)
